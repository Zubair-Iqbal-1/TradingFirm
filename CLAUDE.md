...
- **ai-agent's analyst spends money on a caller-driven route** (Part 4.4). `POST /analyze/{ticker}` makes at most one classifier call and one verdict call; nothing else calls the classifier automatically, so the 40 cap is 40 first-time analyses a day.
  ...
  - **`LLM_PROVIDER_ORDER`** (default empty = OpenRouter's normal routing) pins every request to the listed host(s) with `allow_fallbacks: false`, because a prompt cache is per host. Prod's `.env` sets it; the twin's block does not carry it (`test_provider_order_is_never_set_in_the_twin`). With it set, that host being down is an `LLMUnavailable`, not a reroute.
  ...
  - **`scripts/analyze_live.sh`** is the live check: on the host, against the loopback route, after a go. `LLM_VERDICT_CACHE` stays off until a live call shows `cacheWrite > 0`.
