...
**ai-agent: 8004 is prod** (`tf-ai-agent`, empty `OPENROUTER_API_KEY` — it cannot reach an LLM today); **8014 is its dev twin** (`tf-ai-agent-dev`, `tradingfirm_dev`, Redis DB 1, and three hard-coded locks: empty key, `LLM_DAILY_CALL_CAP=0`, `LLM_BASE_URL=http://openrouter.invalid/api/v1`). Tests run there.
...
docker exec tf-ai-agent-dev pytest tests/test_config.py tests/test_cache.py tests/test_openai_compat_provider.py -v --cov=. --cov-report=term-missing
```
