...
- **Plan math is versioned** (Part 4.8a). `grading/plan_math.PLAN_MATH_VERSION` is stamped on every verdict (`ai.verdicts.plan_math_version`, NULL = 1; `prompt_inputs.planMathVersion`) and sits in the cache fingerprint. **Bump it on any change to a rule**: the T1 ≥ 1.5R walk, `TARGET_MAX_ATR` (6), `FAR_SUPPORT_ATR` (2), `MAX_DISASTER_LOSS_PCT` (2.5), the midpoint re-split, the 1×ATR buffers. Journal stats never average two versions; a rule change without a bump pollutes every era rate.
