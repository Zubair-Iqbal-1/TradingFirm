...
- **Plan math is versioned** (Part 4.8a). `grading/plan_math.PLAN_MATH_VERSION` is stamped on every verdict (`ai.verdicts.plan_math_version`, NULL = 1; `prompt_inputs.planMathVersion`) and sits in the cache fingerprint. **Bump it on any change to a rule**: the T1 ≥ 1.5R walk, `TARGET_MAX_ATR` (8, compared on the distance in ATRs quantized to 2 dp — never a raw float compare at the boundary), `FAR_SUPPORT_ATR` (2), `MAX_DISASTER_LOSS_PCT` (2.5), the 1×ATR buffers. A zone's side is data-engine's label (`side`, tagged by `analyze.tagged_zones`); the midpoint is only the fallback for a zone without one. `held` / `broke` / `lastTouch` pass through to the basis text and decide nothing until plan math v3 (4.8a-de). Journal stats never average two versions; a rule change without a bump pollutes every era rate.
  ...
  - **`compute_plan` takes `ema20`, `swing_low`, `swing_low_date`** (None allowed). The analyst passes `indicators.get("lastSwingLow")` through today, when the dossier has no such key: 4.8a-de adds it on data-engine's side and plan math needs no edit. Both indicator pin tests still list the same keys until then.
