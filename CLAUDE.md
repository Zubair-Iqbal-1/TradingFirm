...
- **The open session's bar is never stored** (4.8b-de): every bar write path goes through `bar_session.drop_open_session_bars` before `upsert_bars`; a new write path must too. The dropped daily row is `sessionSoFar`, attached at read time and never put in a cache body; a market-hours read with no stash or one > 15 min old makes one gated `download_daily([t], "2mo")` (≤ 4 an hour per ticker, never the refresh or its cooldown).
...
- **ai-agent's analyst spends money on a caller-driven route** (Part 4.4). `POST /analyze/{ticker}` makes at most one classifier call and one verdict call; nothing else calls the classifier automatically, so the 40 cap is 40 first-time analyses a day.
  ...
  - **`LLM_VERDICT_CACHE=true` is the measured prod setting** (`.env`, 2026-09-21): the verdict prefix is 2,237 tokens, written on the first AAPL call and read back on the second, both served by Anthropic, $0.0190 → $0.0142. The code default and the twin stay `false`. Editing `prompts/verdict.md` changes the prefix and `prompt_sha`: re-check `cacheWrite` after one. Measured again 2026-09-22 after the units legend: `cacheWrite` 2,585.
  ...
  - **Every number the verdict model reads carries its unit** (spec `verdict-units`, 2026-09-22): a key ending `Atr` / `Pct` / `Frac` / `Usd` / `UsdM`, or a bare price level or one of the conventional keys the legend in `prompts/verdict.md` names. `analyze.INDICATOR_KEYS` is an allowlist, never a pass-through; `profile.marketCapUsdM` is Finnhub's millions. Key names are a contract from now on: 4.7 reads them from `ai.verdicts.prompt_inputs`.
  - **`analyze.PROJECTION_VERSION` bumps on any change to the projected keys.** It is in the cache fingerprint and stored as `prompt_inputs.projectionVersion` (absent = 1). `test_projection_version_is_pinned` fails on a key change without a bump.
  - **The indicator contract exists twice:** ai-agent's `test_indicator_keys_pinned_to_data_engine` and data-engine's `test_indicator_fields_pinned_for_ai_agent` (which also pins `marketCap`'s source and unit). Change both or neither.
- **Plan math is versioned** (Part 4.8a; v3 in 4.8a-de). `grading/plan_math.PLAN_MATH_VERSION` is stamped on every verdict (`ai.verdicts.plan_math_version`, NULL = 1; `prompt_inputs.planMathVersion`) and sits in the cache fingerprint. **Bump it on any change to a rule**: the T1 ≥ 1.5R walk, `TARGET_MAX_ATR` (8, compared on the distance in ATRs quantized to 2 dp — never a raw float compare at the boundary), `FAR_SUPPORT_ATR` (2: the support-zone eligibility, the far branch and the `extended` flag), `CEILING_MIN_HELD` / `CEILING_HELD_PER_BROKE` (3 / 3), `MAX_DISASTER_LOSS_PCT` (2.5), the 1×ATR buffers. A zone's side is data-engine's label (`side`, tagged by `analyze.tagged_zones`); the midpoint is only the fallback for a zone without one. **Levels are built from cent-floored components** (`level_c − atr_c`), so a basis subtraction always equals its level; ratios use the exact ATR. Journal stats never average two versions; a rule change without a bump pollutes every era rate.
  - **Zone history is split by approach side** (`levels.zone_history`, decisions 2026-09-24): `held` / `broke` are totals; `heldBelow` / `brokeBelow` count approaches from below (the level as resistance), `heldAbove` / `brokeAbove` from above. The ceiling reads the below pair (`heldBelow ≥ 3` and `≥ 3 × brokeBelow`); the stop preference reads `heldAbove`, and a support zone with `brokeAbove ≥ heldAbove` is looked through. **Zones are merged while a band stays ≤ 0.5 × ATR14 wide and each side sends the nearest 6 inside `[close − 2.5 ATR, close + 8 ATR]`, filled to 3** (`levels.support_resistance`; the 0.5 % merge and "top 3 by score" are gone).
  - **The zone → target rule is the journal's target rule too:** `scoring.py` judges `target_hit` on `targets[0]`, which is T1 by construction. Don't put a sub-1.5R level back into `targets`; it belongs in `overhead`.
  - **`compute_plan` takes `ema20`, `swing_low`, `swing_low_date`** (None allowed), the swing low being data-engine's `lastSwingLow` (4.8a-de), a far-branch candidate only. Both indicator pin tests list `lastSwingLow`; `touches` / `held` / `broke` are declared conventional keys (counts) and named in the prompt's units sentence.
  - **`plan_view` shows the model text and the two numbers plan math fixed** (`overhead[].price/r/basis`, `targets[].basis`) and never `lossAtDisasterPct`, `sizeShares` or `riskBudget`: a percent of the account is about the account.
  - **`db.journal_rows` reads the ATR from `prompt_inputs.indicators.atr14Usd` or, on rows before the units fix, `atr14`.** Renaming that projected key means editing the query too; `test_stats_by_risk_bucket` pins both names.
