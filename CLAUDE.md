...
- **The open session's bar is never stored** (4.8b-de): every bar write path goes through `bar_session.drop_open_session_bars` before `upsert_bars`; a new write path must too. The dropped daily row is `sessionSoFar`, attached at read time and never put in a cache body.
