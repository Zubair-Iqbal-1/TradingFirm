...
- **`indicators/`** — pure, no-I/O indicator functions, imported from the
  ...
  dataclasses; since Part 4.8a-de over the full stored history, each zone
  with its `touches` / `held` / `broke` / `lastTouch` from `zone_history`,
  plus `last_swing_low`, the newest fractal low), `snapshot.py`
