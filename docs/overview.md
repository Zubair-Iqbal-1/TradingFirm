...
- **`indicators/`** — pure, no-I/O indicator functions, imported from the
  ...
  nodes, merged while a band stays ≤ 0.5 × ATR14 wide (the 0.5 % rule only
  without an ATR), scored 0–90, and since 2026-09-24 each side's nearest 6
  inside [close − 2.5 ATR, close + 8 ATR], filled to 3 from outside,
  returned as `Zone` dataclasses; since Part 4.8a-de over the full stored
